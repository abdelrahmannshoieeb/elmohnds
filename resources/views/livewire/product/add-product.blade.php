                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="grid md:grid-cols-5 gap-3">
                        <div class="">
                            ...
                            <input type="text" id="start-date" class="form-input" wire:model="price"></input>
                            @error('price') <span class="text-red-500">{{ $message }}</span> @enderror

                        </div>
                        <div class="">
                            <label for="start-date" class="mb-2 block" style="font-weight:600;">كمية المنتج في المخزون</label>
                            <input type="text" id="start-date" class="form-input" style="font-weight:600;" wire:model="stock"></input>
                            @error('stock') <span class="text-red-500">{{ $message }}</span> @enderror

                        </div>
                        <div class="">
                            <label for="start-date" class="mb-2 block" style="font-weight:600;">الخصم</label>
                            <input type="text" id="start-date" class="form-input" style="font-weight:600;" wire:model="discount"></input>
                    ...
                    <div class="grid md:grid-cols-4 gap-3">
                        <div>
                            <label for="title" class="mb-2 block" style="font-weight:600;">عنوان الاضافة</label>
                            <input type="text" id="title" class="form-input" wire:model="title">
                            @error('title') <span class="text-red-500">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label for="attribute" class="mb-2 block" style="font-weight:600;">محتوي الاضافة</label>
                            <input type="text" id="attribute" class="form-input" wire:model="attribute">
                            @error('attribute') <span class="text-red-500">{{ $message }}</span> @enderror
                        </div>
                        <button class="btn btn-primary" wire:click="addAttribute">
                            اضف معلومة
                        </button>
                    </div>

                    <div class="mt-4">
                        <h4 style="font-weight:600; font-size: larger" >الاضافات:</h4>
                        <ul>
                            @foreach ($customAttributes as $index => $attr)
                            <li class="flex items-center">
                                <span style="font-size: 20px;">{{ $attr['name'] }}: {{ $attr['value'] }}</span>
                                <button class="text-red-500 ml-3" style="margin: 10px;" wire:click="removeAttribute({{ $index }})"> <li class="mgc_box_2_fill" style="font-size: 20px;"></lic></button>
                            </li>
                            @endforeach
                        </ul>
                    </div>
